...
import { useVerifyOTPRegMutation } from '../slices/usersApiSlice';
...
function Otpscreenforreg() {
  ...
  const [verifyOTPReg, { isLoading }] = useVerifyOTPRegMutation();
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      ...
      if (timeLeft <= 0) {
        ...
        const res = await verifyOTPReg({email,otp: otp.join(''),count: 0});
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      setLoading(true);
      const res = await verifyOTPReg({ email, otp: otp.join(''),count: 5 }).unwrap();
